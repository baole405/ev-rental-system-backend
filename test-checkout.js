...
const testCheckout = async () => {
  try {
    ...
    let booking = await Booking.findOne({ status: BOOKING_STATUS.WAITING_PAYMENT });
    ...
    if (!booking) {
      ...
      const renter = await mongoose.connection.db.collection('users').findOne({ role: 'renter' });
      
      if (!brand || !station || !renter) {
        console.log('❌ Missing required data (brand/station/renter). Run seed first.');
        process.exit(1);
      }
